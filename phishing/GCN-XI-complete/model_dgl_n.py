...
def statistical_information_X(dataset: Xdataset):
    nA, muA, sigma2A = [], [], []
    nF, muF, sigma2F = [], [], []
...
class GCN(nn.Module):
    def __init__(self,in_feats,out_feats,num_of_nodes,mean_A,var_A,var_F,
                 layer_num=1,LRM_layer_num=2,LRM_dimension=120,dropout=0.,
    ...
    def normalize(self, X: torch.TensorType):
        N = 100*torch.nn.functional.normalize(X, p=1, dim=0)
        '''
