...
class EllipticDataset(Dataset):
            ...
            for i in g.nodes():
                if g.nodes[i].data['label'] != -1:
                    ...
                    if len(onehop) + len(twohop) < graph_size:
                        gnidx = torch.cat([gnidx, torch.tensor(onehop).unsqueeze(1), torch.tensor(twohop).unsqueeze(1)],
                                          dim=0)
                    elif len(onehop) < graph_size:
                        gnidx = torch.cat([gnidx, torch.tensor(onehop).unsqueeze(1),
                                           torch.tensor(twohop[:graph_size - len(onehop) - 1]).unsqueeze(1)], dim=0)
                    else:
    ...
    def __getitem__(self, item):
        ...
        #F = 100*torch.nn.functional.normalize(F, p=1, dim=0)
        return A,F,L
